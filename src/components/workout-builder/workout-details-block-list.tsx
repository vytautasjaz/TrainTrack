...
export function WorkoutDetailsBlockList({
  ...
  function addBlock(kind: SmartBlockKind) {
    ...
    const next = appendListedBlock(items, { block: option, section: 'mainSet' })
    commit(next)
  }
  ...
  return (
    <div className="min-w-0 space-y-1.5">
      {items.length === 0 && (
        <p className="py-1 text-sm text-muted-foreground">No blocks yet — add one below.</p>
      )}
        ...
        return (
          ...
          >
            <div className="flex items-stretch">
              ...
              >
                <div className={cn('w-1 shrink-0', accent.stripe)} aria-hidden />
                <span
                  draggable
                  onDragStart={(e) => {
                    e.stopPropagation()
