...
function formatTemplateMeta(t: TrainingLibraryTemplateItem): string {
  const parts: string[] = []
  if (t.plannedDistanceMeters != null && t.plannedDistanceMeters > 0) {
    parts.push(`${Math.round(t.plannedDistanceMeters)} m`)
  } else if (t.distanceKm != null && t.distanceKm > 0) {
    parts.push(`${t.distanceKm} km`)
  }
  if (t.durationMin != null && t.durationMin > 0) {
    parts.push(formatDuration(t.durationMin))
  }
...
export function PlanCanvasLibraryPanel() {
  ...
  const canDropSession = dnd?.dragItem?.kind === 'session'
  const showDropHint = Boolean(canDropSession)

  function resolveDropFolderId(sessionSport: WorkoutType): string | null {
    if (folderFilter === 'all' || folderFilter === 'unfiled') return null
    const folder = library?.folders.find((f) => f.id === folderFilter)
    if (!folder || folder.sport !== sessionSport) return null
    return folder.id
  }
  ...
  function handleDrop(e: React.DragEvent) {
    ...
    e.stopPropagation()
    setIsOver(false)
    if (!canDropSession || !dnd?.dragItem || dnd.dragItem.kind !== 'session') {
      return
    }
    const folderId = resolveDropFolderId(dnd.dragItem.sport)
    dnd.savePlanSessionToLibrary(dnd.dragItem.id, folderId)
  }
  ...
  return (
    <aside
      ...
      className={cn(
        ...
        canDropSession &&
          !isOver &&
          'bg-[color-mix(in_srgb,var(--tt-red,#da2f36)_4%,white)]',
        canDropSession &&
          isOver &&
          'bg-[color-mix(in_srgb,var(--tt-red,#da2f36)_8%,white)]',
      )}
    >
        ...
        {showDropHint && filtered.length === 0 ? (
          <li className="list-none px-1 py-8 text-center text-xs font-medium text-[var(--tt-red,#da2f36)]">
            ...
            {folderFilter !== 'all' && folderFilter !== 'unfiled'
              ? ` · ${folderNameById.get(folderFilter) ?? 'folder'}`
              : ''}
          </li>
        ) : filtered.length === 0 ? (
          ...
          filtered.map((t) => (
            <TemplateRow
              ...
              template={t}
              folderLabel={
                t.folderId ? (folderNameById.get(t.folderId) ?? null) : null
              }
            />
          ))
        )}
