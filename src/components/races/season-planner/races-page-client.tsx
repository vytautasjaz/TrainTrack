...
import {
  ...
  AddRaceModal,
  WatchRaceButton,
} from "@/components/races/add-race-modal";
...
function SeasonPlannerView({
  ...
  const [raceColorMode, setRaceColorMode] =
    useState<PlannerRaceColorMode>("priority");
  const [priorityFilter, setPriorityFilter] = useState<
    Record<RacePriority, boolean>
  >({
    A: true,
    B: true,
    C: true,
      ...
      <div className="flex flex-nowrap items-center gap-x-2 overflow-x-auto pb-0.5">
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <span className="hidden text-[10px] font-semibold uppercase tracking-[0.06em] text-muted-foreground sm:inline">
              Color
            </span>
            <SegmentedControl aria-label="Race color" className="shrink-0">
              {(
                [
                  ["priority", "Priority"],
                  ["sport", "Sport"],
                ] as const satisfies readonly [PlannerRaceColorMode, string][]
              ).map(([id, label]) => (
                <SegmentedControlItem
                  key={id}
                  active={raceColorMode === id}
                  className="px-3 text-xs"
                  onClick={() => setRaceColorMode(id)}
                >
                  {label}
                </SegmentedControlItem>
              ))}
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      <FilterGroupAllNone
                        ...
                        onAll={() =>
                          setPriorityFilter({ A: true, B: true, C: true })
                        }
                        onNone={() =>
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      <FilterGroupAllNone
                        allSelected={PLANNER_SPORTS.every(
                          (s) => sportFilter[s],
                        )}
                        noneSelected={PLANNER_SPORTS.every(
                          (s) => !sportFilter[s],
                        )}
                        onAll={() =>
                          setSportFilter(
...
function scrollPlannerToColumn(
  columnIndex: number,
  colW: number,
  labelW: number,
) {
  const el = document.getElementById("season-planner-scroller");
  if (!el) return;
  const target = plannerScrollLeftForColumn(
    columnIndex,
    colW,
    el.clientWidth,
    labelW,
...
function SeasonPlannerBoard({
  ...
  raceColorMode: PlannerRaceColorMode;
  phaseBlocks: SeasonPhaseBlockData[];
  today: Date;
  ...
  // Day-zoom only: pin prep name + week label inside each week while scrolling.
  useLayoutEffect(() => {
    if (unit !== "day") return;
    const scroller = document.getElementById("season-planner-scroller");
    if (!scroller) return;
    ...
    const clearTransforms = () => {
      scroller
        .querySelectorAll<HTMLElement>("[data-prep-week-label]")
        .forEach((el) => {
          el.style.transform = "";
        });
    };
    ...
    const update = () => {
      ...
      scroller
        .querySelectorAll<HTMLElement>("[data-prep-week-label]")
        .forEach((el) => {
          const left = Number(el.dataset.prepLeft);
          const width = Number(el.dataset.prepWidth);
          if (!Number.isFinite(left) || !Number.isFinite(width)) return;
          const labelWidth = el.offsetWidth;
          const maxOffset = Math.max(0, width - labelWidth - 4);
          const desired = scrollLeft - left;
          const offset = Math.min(Math.max(0, desired), maxOffset);
          el.style.transform = offset ? `translateX(${offset}px)` : "";
        });
    };
  ...
  return (
    <div className={cn(TABLE_SHELL, "tt-season-timeline")}>
      ...
      >
        <div
          style={{ width: labelW + gridW, minWidth: "100%" }}
          className="relative"
        >
          {/* Month header */}
            ...
            <div className="relative" style={{ width: gridW }}>
              {Array.from({ length: colCount }, (_, i) => (
                <div
                  ...
                  data-week-start={
                    unit === "day" && scale.isWeekBoundary(i)
                      ? "true"
                      : undefined
                  }
                  data-weekend={
          ...
          {visiblePriorityLanes.map(({ priority, label }) => {
            const laneRaces = races.filter(
              (r) =>
                r.priority === priority && r.intent !== RaceIntent.WATCHING,
            );
            return (
              <PlannerLane
                key={priority}
                ...
                colW={colW}
                scale={scale}
                contentMinHeight={raceLaneMinHeight(
                  laneRaces,
                  weeks,
                  scale,
                  colW,
          ...
          {visibleSports.map((sport) => {
            ...
            const raceLayoutHeight = raceLaneMinHeight(
              sportRaces,
              weeks,
              scale,
              colW,
      ...
      <div className="relative" style={{ width: gridW, minHeight: minH }}>
        {Array.from({ length: colCount }).map((_, i) => (
          <div
            ...
            data-month-end={scale.isMonthEnd(i) ? "true" : undefined}
            data-week-start={
              scale.unit === "day" && scale.isWeekBoundary(i)
                ? "true"
                : undefined
            }
            data-weekend={
...
function raceCardHeight(colW: number, unit?: PlannerZoomUnit) {
  // Day zoom: prep phase labels need vertical room even though day cols are narrow.
  if (unit === "day") return Math.max(RACE_CARD_HEIGHT_ROOMY, 52);
  return raceCardRoomy(colW)
    ? RACE_CARD_HEIGHT_ROOMY
    : RACE_CARD_HEIGHT_COMPACT;
}

function eventCardHeight(colW: number) {
  return raceCardRoomy(colW)
    ? EVENT_CARD_HEIGHT_ROOMY
    : EVENT_CARD_HEIGHT_COMPACT;
}
...
function layoutStackedRaces(
  ...
  const items = [...races]
    .map((race) => {
      ...
      if (win != null) {
        if (scale.unit === "day") {
          const daySpan = prepWindowDaySpan(win, weeks, scale.days, race.date);
...
function raceLaneMinHeight(
  ...
  if (races.length === 0)
    return raceCardRoomy(colW) || scale.unit === "day" ? 62 : 46;
  const { rows } = layoutStackedRaces(races, weeks, scale, colW);
  const cardH = raceCardHeight(colW, scale.unit);
  return (
    RACE_CARD_TOP +
    rows * cardH +
...
function buildPrepPhaseSegments(
  prepWin: { startWeekIndex: number; endWeekIndex: number },
  prepBlocks: RacePrepBlock[] | null,
): Array<{
  startWeekIndex: number;
  weekCount: number;
  block: RacePrepBlock | null;
}> {
  // Include race week — last week starts Monday and ends on race day.
  const totalPrep = prepWin.endWeekIndex - prepWin.startWeekIndex + 1;
  ...
  for (let i = 0; i < totalPrep; i++) {
    const block = prepBlockForWeekOffset(prepBlocks, i, totalPrep);
    const weekIdx = prepWin.startWeekIndex + i;
    const last = segments[segments.length - 1];
    const same =
      last &&
      last.startWeekIndex + last.weekCount === weekIdx &&
      ((block == null && last.block == null) ||
        (block != null &&
          last.block != null &&
          block.phase === last.block.phase &&
          (block.label ?? null) === (last.block.label ?? null)));
    if (same && last) {
      last.weekCount += 1;
    } else {
      segments.push({ startWeekIndex: weekIdx, weekCount: 1, block });
    }
  }
  return segments;
}

/** Pixel width of one prep week in the trail (race week is Mon→race day). */
function prepWeekTrailWidthPx(args: {
  weekIdx: number;
  raceWeekIdx: number;
  raceDate: Date;
  weeks: PlannerWeekColumn[];
  scale: PlannerBoardScale;
  colW: number;
  /** Include race week in the trail (day-column / week-level zooms only). */
  includeRaceWeek: boolean;
}): number {
  const {
    weekIdx,
    raceWeekIdx,
  ...
  if (weekIdx !== raceWeekIdx) {
    return scale.unit === "day" ? 7 * colW : colW;
  }
  // Race week (often a 1w taper): only draw where Mon→race day is readable.
  if (!includeRaceWeek) return 0;
  if (scale.unit === "day") {
    const week = weeks[weekIdx];
    if (!week) return 0;
    const monIdx = dayIndexForDate(scale.days, week.start);
    const raceIdx = dayIndexForDate(scale.days, raceDate);
    return Math.max(0, (raceIdx - monIdx) * colW);
  }
  return 0;
}
...
function StackedRaceCards({
  ...
  const cardH = raceCardHeight(colW, scale.unit);
  const roomy = raceCardRoomy(colW) || scale.unit === "day";
  const follow = usePlannerFollowTooltip();
  const weekColSpan = scale.unit === "day" ? 7 : 1;
  ...
  return (
    <>
      {items.map(({ race, left, width, stack, prepWin }) => {
        ...
        const chrome = plannerRaceChrome(race, raceColorMode);
        // Prep window always includes race week (Mon→race day). Show capsule
        // whenever preparation weeks are set — including a 1w taper.
        const showPrep = prepWin != null;
        ...
        const prepStartWeek = weeks[prepWin.startWeekIndex];
        const prepLeft =
          scale.unit === "day" && prepStartWeek
            ? dayIndexForDate(scale.days, prepStartWeek.start) * colW
            : prepWin.startWeekIndex * colW;
        const capsuleLeft = prepLeft + 1;
        // Last week is often a single taper block (Mon→race day). Only show it
        // at week-level day zooms (2 wk / 1 wk) where those days are visible.
        const includeRaceWeek = scale.unit === "day";
        const capsuleWidth = Math.max(left + width - capsuleLeft, width);
        const prepWidth = Math.max(left - capsuleLeft, 0);
        const segments = buildPrepPhaseSegments(prepWin, prepBlocks);
        const weekWidths = Array.from(
          {
            length: prepWin.endWeekIndex - prepWin.startWeekIndex + 1,
          },
          (_, i) => {
            const weekIdx = prepWin.startWeekIndex + i;
            return prepWeekTrailWidthPx({
              weekIdx,
              raceWeekIdx: prepWin.endWeekIndex,
              raceDate: race.date,
              weeks,
              scale,
              colW,
        ...
        const weekPx = weekColSpan * colW;
        // Day zooms keep large type; week zooms sit between compact and day size.
        const typeScale =
          scale.unit === "day"
            ? "xl"
            : weekPx >= 56
        ...
        const nameClass =
          typeScale === "xl"
            ? "px-2.5 pt-1.5 text-[14px] font-normal tracking-tight"
            : typeScale === "lg"
              ? "px-2 pt-1.5 text-[12px] font-normal tracking-tight"
              : typeScale === "md"
                ? "px-1.5 pt-1 text-[11px] font-normal tracking-tight"
                : "px-1 pt-0.5 text-[10px] font-normal tracking-tight";
        const labelPadClass =
          typeScale === "xl"
            ? "px-2.5"
            : typeScale === "lg"
        ...
        return (
          ...
          >
            {/* Soft outline + fill grows through prep into the race pill. */}
            <div
              className={cn(
                "pointer-events-none absolute inset-0 rounded-[6px] border",
                isWatching
            ...
            <div className="relative flex h-full w-full items-stretch">
              <div
                className={cn(
                  "flex min-w-0 bg-white/30",
                  isWatching ? "text-muted-foreground" : chrome.ink,
                )}
                style={{ width: Math.max(prepWidth, 0) }}
                aria-hidden
              >
                {(() => {
                  let weekOffsetPx = 0;
                  return segments.map((seg, segIndex) => {
                    const name =
                      seg.block && !isRaceWeekPrepBlock(seg.block)
                        ? prepBlockDisplayName(seg.block)
                        : null;
                    let segPx = 0;
                    for (let wi = 0; wi < seg.weekCount; wi++) {
                      const off =
                        seg.startWeekIndex - prepWin.startWeekIndex + wi;
                      segPx += weekWidths[off] ?? 0;
                    }
                    if (segPx < 1) return null;
                    const showName = Boolean(name) && segPx >= 20;
                    // Day zoom: name + week # in every week, sticky within that week.
                    // Week zoom: one centered name + plain centered week numbers.
                    const labelEachWeek = scale.unit === "day";
                    const nameOnce = showName && !labelEachWeek;
                    const phaseDivider =
                      segIndex > 0
                        ? isWatching
                          ? "border-l border-foreground/20"
                          : chrome.dividerBorder
                        : null;

                    return (
                      <div
                        key={`${race.id}-prep-${scale.unit}-${seg.startWeekIndex}`}
                        className={cn(
                          "relative flex min-w-0 flex-col",
                          phaseDivider,
                        )}
                        style={{ width: segPx, flex: "0 0 auto" }}
                        title={
                          name
                            ? `${name} · prep for ${race.name}`
                            : `Prep for ${race.name}`
                        }
                      >
                        {nameOnce ? (
                          <div
                            className={cn(
                              "pointer-events-none absolute inset-x-0 top-0 z-[1] truncate text-center leading-none opacity-70",
                              nameClass,
                            )}
                          >
                            {name}
                        ...
                        <div
                          className={cn(
                            "relative z-0 flex min-h-0 flex-1",
                            labelEachWeek
                              ? "items-stretch"
                              : nameOnce
                                ? "items-end pb-1"
                        ...
                        >
                          {Array.from({ length: seg.weekCount }, (_, wi) => {
                            const weekIdx = seg.startWeekIndex + wi;
                            const off = weekIdx - prepWin.startWeekIndex;
                            const wPx = weekWidths[off] ?? 0;
                            const weekLeft = capsuleLeft + weekOffsetPx;
                            weekOffsetPx += wPx;
                            if (wPx < 1) return null;
                            const weeksLeft =
                              prepWin.endWeekIndex - weekIdx + 1;
                            const weeksLeftLabel =
                              weeksLeft === 1 ? (
                                <span className="opacity-45">RACE WEEK</span>
                              ) : (
                                <>
                                  <span className="font-semibold opacity-70">
                                    {weeksLeft} weeks
                                  </span>
                                  <span className="opacity-45">{` to ${race.name}`}</span>
                                </>
                              );
                            const weekShowName =
                              labelEachWeek && Boolean(name) && wPx >= 28;
                            const phaseWeekLabel =
                              name && seg.weekCount > 1
                                ? `${name} (${wi + 1}/${seg.weekCount})`
                                : name;

                            if (!labelEachWeek) {
                              return (
                                <div
                                  key={`${race.id}-prep-w-week-${weekIdx}`}
                                  className="flex min-w-0 items-center justify-center px-0.5"
                                  style={{ width: wPx, flex: "0 0 auto" }}
                                >
                                  <span
                                    className={cn(
                                      "text-center leading-none opacity-45",
                                      countClass,
                                    )}
                                  >
                                    {weeksLeft}
                            ...
                            return (
                              <div
                                key={`${race.id}-prep-w-day-${weekIdx}`}
                                className="relative min-w-0 overflow-hidden"
                                style={{ width: wPx, flex: "0 0 auto" }}
                              >
                                <div
                                  data-prep-week-label
                                  data-prep-left={weekLeft}
                                  data-prep-width={wPx}
                                  className="pointer-events-none absolute left-0 top-0 z-[1] flex h-full w-max max-w-full flex-col justify-between"
                                >
                                  {weekShowName ? (
                                    <div
                                      className={cn(
                                        "truncate leading-none opacity-70",
                                        nameClass,
                                      )}
                                    >
                                      {phaseWeekLabel}
                                  ...
                                  <div
                                    className={cn(
                                      "max-w-full truncate pb-1 text-left leading-none",
                                      labelPadClass,
                                      countClass,
                                    )}
                                  >
...
function raceFollowTipContent(race: SeasonRace) {
  ...
  return (
    <>
      ...
      {distance ? (
        <PlannerTipRow label="Distance">{distance}</PlannerTipRow>
      ) : null}
      {race.location?.trim() ? (
...
function RaceCard({
  ...
  /** When true, race sits as a pill at the end of a lighter prep trail. */
  attached?: boolean;
}) {
  const isWatching = race.intent === RaceIntent.WATCHING;
  ...
  return (
    <button
      ...
      className={cn(
        "overflow-hidden px-1.5 py-0.5 text-left transition hover:brightness-[0.98]",
        attached
          ? cn(
              "relative z-[2] h-full shrink-0 rounded-[6px] border",
              isWatching
                ? "border-dashed border-foreground/25 bg-muted/50 text-foreground shadow-none"
                : cardClass,
            )
          : cn(
              "absolute z-[3] rounded-[6px] border",
              isWatching
                ? "border-dashed border-foreground/25 bg-muted/40 text-foreground shadow-none"
                : cardClass,
            ),
      )}
      style={attached ? { width, height } : { left, width, top, height }}
    >
      <p className="truncate text-[9px] font-medium leading-none opacity-70">
...
function RaceListSection({
  ...
  const [sportFilter, setSportFilter] =
    useState<Record<PlannerSport, boolean>>(defaultSportFilter);
  const [priorityFilter, setPriorityFilter] = useState<
    Record<RacePriority, boolean>
...
function SeasonRaceTable({
  ...
  const priorityFilterActive = Object.values(filters.priorityFilter).some(
    (v) => !v,
  );
  ...
  return (
    <div className={DATA_TABLE_SHELL}>
      <div className="hidden overflow-x-auto md:block">
        <table
          className={cn(DATA_TABLE, "min-w-[40rem]")}
              ...
              <th>
                <span className="inline-flex items-center gap-0.5">
                  ...
                  <HeaderFilterMenu
                    ...
                    allSelected={Object.values(filters.sportFilter).every(
                      Boolean,
                    )}
                    noneSelected={Object.values(filters.sportFilter).every(
                      (v) => !v,
                    )}
                  >
                    {PLANNER_SPORTS.map((sport) => (
              ...
              <th>
                <span className="inline-flex items-center gap-0.5">
                  ...
                  <HeaderFilterMenu
                    ...
                    allSelected={Object.values(filters.priorityFilter).every(
                      Boolean,
                    )}
                    noneSelected={Object.values(filters.priorityFilter).every(
                      (v) => !v,
                    )}
                  >
                    {PLANNER_PRIORITY_LANES.map(({ priority }) => (
              ...
              sortedRaces.map((race) => {
                ...
                return (
                  <tr key={race.id} className={cn(isPast && "opacity-80")}>
                    <td
                      className={cn("whitespace-nowrap", DATA_CELL_SECONDARY)}
                    >
                      {race.date.toLocaleDateString(undefined, {
                        month: "short",
                        day: "numeric",
        ...
        onConfirm={() => {
          ...
          startTransition(async () => {
            try {
              ...
              setDeleteError(
                err instanceof Error ? err.message : "Could not delete race",
              );
            }
          });
        }}
