...
function formatIncludeSegmentCompact(segment?: Segment): string {
  if (!segment || !Number.isFinite(segment.value) || segment.value <= 0) return ''
 ...
 */
export function formatIncludeItemsForSubtitle(items: WorkoutIncludeItem[]): string {
...
export function IncludeItemsSummary({ items, compact = false }: IncludeItemsSummaryProps) {
  if (items.length === 0) return null

  return (
    <div className={compact ? 'space-y-1.5' : 'space-y-2'}>
      {items.map((item) => (
        <div key={item.id} className={compact ? 'text-xs text-muted-foreground' : 'text-sm text-muted-foreground'}>
          <span className="font-semibold text-foreground">{item.title}</span>{' '}
          <span>
            {item.repetitions} x {formatSegment(item.work.value, item.work.unit)}
