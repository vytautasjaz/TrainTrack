  ...
  if (entry.prefix) {
    return (
      <p className={cn("leading-snug text-muted-foreground", className)}>
        <span className="font-semibold text-foreground">{entry.prefix}</span>{" "}
        {entry.text}
      </p>
    );
  }
...
export function WorkoutPlanDetails({
  ...
  const descSize = isCompact
    ? "text-[9px] line-clamp-1"
    : "text-[11px] line-clamp-2";

  const rescheduleLabel = showRescheduleLabel
    ? getReschedulePlanLabel(workout)
    : null;
    ...
    <div className={cn("space-y-0.5", className)}>
      ...
      {primaryMetrics && (
        <p
          className={cn(
            metricSize,
            "font-bold tabular-nums leading-snug text-foreground",
          )}
        >
          {primaryMetrics}
        </p>
      ...
      {plannedComparison && (
        <p className={cn(plannedSize, "text-muted-foreground")}>
          {plannedComparison}
        </p>
      )}
      ...
      {workout.structure &&
        (hasStructureContent(workout.structure) ||
          hasIncludeItems(workout.structure)) && (
      ...
      {workout.structure?.includeItems &&
      workout.structure.includeItems.length > 0 ? (
        <div className={cn(isCompact ? "pt-1" : "pt-1.5")}>
          <p className={cn(descSize, "mb-1 font-semibold text-foreground")}>
            Include
          </p>
