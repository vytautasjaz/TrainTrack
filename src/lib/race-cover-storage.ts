...
const COVER_DIR = ['public', 'uploads', 'race-covers'] as const
...
async function blobStore() {
  try {
    ...
    if (siteID && token) {
      return getStore({ name: 'race-covers', siteID, token, consistency: 'strong' })
    }
    return getStore({ name: 'race-covers', consistency: 'strong' })
  } catch {
    return null
  }
}
...
/** Persist a race cover so it survives Netlify deploys (Blobs in prod, disk locally). */
export async function putRaceCoverFile(
  filename: string,
  buffer: Buffer,
  contentType: string,
) {
...
export async function getRaceCoverFile(
  filename: string,
